...
SELECT * FROM v_users;
";
        var q = SqlQueryParser.ParseMulti(sql, new NpgsqlDialect(version)).ToList();
    ...
    public void Parse_CreateOrReplaceView_ShouldSetFlag(int version)
    {
        ...
        var q = SqlQueryParser.ParseMulti(sql, new NpgsqlDialect(version)).Single();
    ...
    {
        Assert.Throws<InvalidOperationException>(() => SqlQueryParser.Parse("DROP VIEW ;", new NpgsqlDialect(version)));
        Assert.Throws<InvalidOperationException>(() => SqlQueryParser.Parse("DROP VIEW IF EXISTS ;", new NpgsqlDialect(version)));
    }
