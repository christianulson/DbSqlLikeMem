    ...
    ) : XUnitTestBase(helper)
{
    ...
    public void ExecuteNonQuery_DELETE_remove_varias_linhas()
    {
        ...
        using var conn = NewConn(threadSafe: false, db);
        using var cmd = new NpgsqlCommandMock(conn) { CommandText = "DELETE FROM users WHERE id = 2" };
    ...
    public void ExecuteNonQuery_DELETE_quando_nao_acha_retorna_0()
    {
        ...
        using var conn = NewConn(threadSafe: false, db);
        using var cmd = new NpgsqlCommandMock(conn) { CommandText = "DELETE FROM users WHERE id = 999" };
    ...
    public void ExecuteNonQuery_DELETE_bloqueia_quando_fk_referencia()
    {
        ...
        using var conn = NewConn(threadSafe: false, db);
        using var cmd = new NpgsqlCommandMock(conn) { CommandText = "DELETE FROM parent WHERE id = 10" };
    ...
    public void ExecuteNonQuery_DELETE_com_parametro_se_suportado()
    {
        ...
        using var conn = NewConn(threadSafe: false, db);
        using var cmd = new NpgsqlCommandMock(conn)
        {
            CommandText = "DELETE FROM users WHERE id = @id"
        };

        cmd.Parameters.Add(new NpgsqlParameter("@id", 2));
    ...
    private static NpgsqlConnectionMock NewConn(bool threadSafe, NpgsqlDbMock db)
    {
        db.ThreadSafe = threadSafe;

        var conn = new NpgsqlConnectionMock(db);
        return conn;
    }

    private static ITableMock NewUsersTable(NpgsqlDbMock db)
    {
    ...
    private static ITableMock NewParentTable(NpgsqlDbMock db)
    {
    ...
    private static ITableMock NewChildTable(NpgsqlDbMock db)
    {
