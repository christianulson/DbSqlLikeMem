...
#pragma warning restore CA1859 // Use concrete types when possible for improved performance
    ...
    {
        var collector = new OracleTranslationParameterCollector();
...
#pragma warning restore CS8605 // Unboxing a possibly null value.
    ...
    protected override Expression VisitConstant(ConstantExpression node)
    {
        ...
        // 1) Prioridade: se for OracleQueryable<>, usa TableName (é o que você passou em AsQueryable<T>("users"))
        if (node.Value is not null)
        {
            var t = node.Value.GetType();
            if (t.IsGenericType && t.GetGenericTypeDefinition() == typeof(OracleQueryable<>))
            {
            ...
            case ConstantExpression constant:
                {
                    ...
                    if (valueType.IsGenericType && valueType.GetGenericTypeDefinition() == typeof(OracleQueryable<>))
                    {
                        var property = valueType.GetProperty(nameof(OracleQueryable<object>.TableName), BindingFlags.Instance | BindingFlags.Public);
    ...
    private sealed class OracleTranslationParameterCollector : ExpressionVisitor
    {
