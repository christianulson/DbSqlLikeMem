    ...
    ) : XUnitTestBase(helper)
{
    ...
    public void TestFluent()
    {
        ...
        var count = cnn.Db.GetTable("user").Count;   // 2 linhas

        var idIdx = cnn.Db.GetTable("user").Columns["id"].Index;     // 0
        var firstId = cnn.Db.GetTable("user")[0][idIdx];             // 1 (auto-increment)
    }
}
