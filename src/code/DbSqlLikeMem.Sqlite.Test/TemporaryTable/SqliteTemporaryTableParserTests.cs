...
/// <summary>
/// EN: Covers CREATE TEMPORARY TABLE parsing scenarios in the Sqlite dialect.
/// PT-br: Cobre cenarios de parsing de CREATE TEMPORARY TABLE no dialeto Sqlite.
/// </summary>
...
";
        ...
        select2.Table!.Name.Should().Be("tmp_users");
    }
...
WHERE `tenantid` = 10",
    ...
    /// PT-br: Verifica se CREATE GLOBAL TEMPORARY TABLE e parsed com escopo global.
    /// </summary>
    [Theory]
    [Trait("Category", "TemporaryTable")]
    [MemberDataSqliteVersion]
    ...
    public void Parse_ShouldAccept_DropTable_IfExists(int version)
    {
        ...
        var q = SqlQueryParser.Parse("DROP TABLE IF EXISTS tmp_users", db, d)
            .Should().BeOfType<SqlDropTableQuery>().Which;

        q.IfExists.Should().BeTrue();
        q.Table.Should().NotBeNull();
        q.Table!.Name.Should().Be("tmp_users");
    }
