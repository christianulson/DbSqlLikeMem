...
internal static class AutoValueHelper
{
        ...
        IReadOnlyDictionary<string, ColumnDef>? colDict = null)
    {
        ...
        if (token.StartsWith("@"))
        {
            ...
            if (pars == null || !pars.Contains(name))
                throw new AutoMockException(SqlExceptionMessages.ParameterNotFound(name));
            return ((DbParameter)pars[name]).Value;
        }
        ...
        IReadOnlyDictionary<string, ColumnDef>? colDict)
    {
        ...
        if (cdef.Size is int size && value is string s && s.Length > size)
            throw new AutoMockException(SqlExceptionMessages.DataTooLongForColumn(CurrentColumn!), 1406);

        if (cdef.DecimalPlaces is int scale && value is decimal d && GetDecimalScale(d) > scale)
            throw new AutoMockException(SqlExceptionMessages.DataTruncatedForColumn(CurrentColumn!), 1265);
