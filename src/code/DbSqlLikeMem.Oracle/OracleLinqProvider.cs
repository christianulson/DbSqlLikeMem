...
/// PT-br: Fornece tradução e execução de consultas LINQ para a conexão simulada Oracle.
/// </summary>
public sealed class OracleQueryProvider(
    OracleConnectionMock cnn
    ) : IQueryProvider
{
    private readonly OracleConnectionMock _cnn = cnn ?? throw new ArgumentNullException(nameof(cnn));
    ...
    public IQueryable CreateQuery(Expression expression)
    {
        ...
        var queryType = typeof(OracleQueryable<>).MakeGenericType(elementType);
    ...
    public IQueryable<TElement> CreateQuery<TElement>(Expression expression)
    {
        ArgumentNullExceptionCompatible.ThrowIfNull(expression, nameof(expression));
        var tableName = ExtractTableName(expression);
        return new OracleQueryable<TElement>(this, expression, tableName);
    }
    ...
    private static string ExtractTableName(Expression expression)
    {
        switch (expression)
        {
            // 1) Se for um ConstantExpression cuja Value é OracleQueryable<AlgumTipo>
            case ConstantExpression c:
                {
                    ...
                    if (val != null)
                    {
                        ...
                        if (valType.IsGenericType &&
                            valType.GetGenericTypeDefinition() == typeof(OracleQueryable<>))
                        {
                            // Pega a propriedade pública TableName por reflection
                            var prop = valType.GetProperty(
                                nameof(OracleQueryable<object>.TableName),
