...
/// <summary>
/// EN: Covers extra SQL Server SELECT and WHERE scenarios over a direct mock connection.
/// PT-br: Cobre cenarios extras de SELECT e WHERE SQL Server sobre uma conexao mock direta.
/// </summary>
public sealed class SqlServerSelectAndWhereMoreCoverageTests : XUnitTestBase
{
    ...
    /// <summary>
    /// EN: Creates the in-memory SQL Server database used by the extra SELECT and WHERE coverage tests.
    /// PT-br: Cria o banco SQL Server em memoria usado pelos testes extras de cobertura de SELECT e WHERE.
    /// </summary>
...
ORDER BY id").ToList();
    ...
    /// <summary>
    /// EN: Verifies ISNULL or COALESCE-style projections return the expected fallback value.
    /// PT-br: Verifica se projeções no estilo ISNULL ou COALESCE retornam o valor de fallback esperado.
    /// </summary>
    [Fact]
    [Trait("Category", "SqlServerSelectAndWhereMoreCoverage")]
