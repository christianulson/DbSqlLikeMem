...
/// PT-br: Fornece tradução e execução de consultas LINQ para a conexão simulada SQL Server.
/// </summary>
public sealed class SqlServerQueryProvider(
    SqlServerConnectionMock cnn
    ) : IQueryProvider
{
    private readonly SqlServerConnectionMock _cnn = cnn ?? throw new ArgumentNullException(nameof(cnn));
    ...
    private static string ExtractTableName(Expression expression)
    {
        switch (expression)
        {
            // 1) Se for um ConstantExpression cuja Value é SqlServerQueryable<AlgumTipo>
            case ConstantExpression c:
                {
                    ...
                    if (val != null)
                    {
                        ...
                        if (valType.IsGenericType &&
                            valType.GetGenericTypeDefinition() == typeof(SqlServerQueryable<>))
                        {
                            // Pega a propriedade pública TableName por reflection
                            var prop = valType.GetProperty(
                                nameof(SqlServerQueryable<object>.TableName),
