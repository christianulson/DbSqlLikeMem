...
/// <summary>
/// EN: Covers parser corpus cases for the SqlServer dialect.
/// PT-br: Cobre casos de corpus do parser para o dialeto SqlServer.
/// </summary>
...
) : XUnitTestBase(helper)
{
    ...
    public static IEnumerable<object[]> SelectStatements()
    {
        ...
        // DISTINCT
        yield return new object[] { "SELECT DISTINCT id FROM t ORDER BY id DESC OFFSET 1 ROWS FETCH NEXT 2 ROWS ONLY", "SELECT DISTINCT + ORDER BY + LIMIT/OFFSET" };
        ...
        yield return new object[] { "SELECT * FROM t ORDER BY iddesc ASC OFFSET 1 ROWS FETCH NEXT 2 ROWS ONLY", "ORDER BY ASC + LIMIT/OFFSET" };
        ...
        yield return new object[] { "SELECT id, ISNULL(email, 'none') AS em FROM users ORDER BY id", "IFNULL(a,b)" };
...
GROUP BY grp", "GROUP BY with multiple aggregates" };
        yield return new object[] { "SELECT grp, COUNT(val) AS C FROM t GROUP BY grp HAVING C > 1", "HAVING using alias (MySQL allows)" };
...
ORDER BY u.Id, o.Amount", "JOIN with derived subquery + ORDER BY multiple keys" };
        ...
        yield return new object[] { "select JSON_VALUE(data, '$.name') from users", "JSON_VALUE function call" };
...
", "WITH inside derived table + outer WHERE" };
    ...
    "LIMIT with count only"
        ...
        yield return new object[] { "DELETE FROM users WHERE id = 1", "DELETE without FROM keyword (MySQL syntax)" };
        ...
        yield return new object[] { "INSERT INTO t () VALUES ()", "NSERT default row (no columns, no values)" };
...
USING (SELECT id, name FROM users_tmp) AS src
ON target.id = src.id
WHEN MATCHED THEN UPDATE SET target.name = src.name
WHEN NOT MATCHED THEN INSERT (id, name) VALUES (src.id, src.name);",
            "MERGE upsert from SELECT source"
        };

        yield return new object[] {
            @"MERGE INTO stats AS target
USING (SELECT grp, SUM(val) AS total FROM t GROUP BY grp) AS src
ON target.grp = src.grp
WHEN MATCHED THEN UPDATE SET target.total = src.total
WHEN NOT MATCHED THEN INSERT (grp, total) VALUES (src.grp, src.total);",
            "MERGE upsert with aggregate source"
        };
        yield return new object[] {
            "MERGE INTO users u USING tmp t ON u.id = t.id WHEN MATCHED THEN UPDATE SET u.name = t.name", "Merge"
        };
    }
    ...
    {
        // MERGE
        yield return new object[] { "select data->'$.name' from users", "MySQL JSON -> operator" };
        yield return new object[] { "select data->>'$.name' from users", "MySQL JSON ->> operator" };
        ...
        yield return new object[] { @"
select id
     , (SELEC UserId FROM user_roles R WHERE U.id = R.userId ORDER BY UserId FETCH FIRST 1 ROWS ONLY ) 
  from shcema1.users U", "invalid: typo SELEC" };

        yield return new object[] { @"
select id
     , (SELEC UserId2 FROM user_roles R WHERE U.id = R.userId ORDER BY 1 FETCH FIRST 1 ROWS ONLY ) 
  from shcema1.users U", "invalid: typo SELEC" };
        // DISTINCT duplicado
        yield return new object[] {
...
#pragma warning disable CA1031 // Do not catch general exception types
        ...
        catch (NotSupportedException e)
        {
            ...
            Assert.True(expectation == SqlCaseExpectation.ThrowNotSupported,
                $"Esperava {expectation} mas veio NotSupported. Why={why}. Version={version}");
        }
        catch (Exception e)
        {
            Console.WriteLine($"Exception: {e}");
            Assert.True(expectation == SqlCaseExpectation.ThrowInvalid,
                $"Esperava {expectation} mas veio Exception. Why={why}. Version={version}");
        }
#pragma warning restore CA1031 // Do not catch general exception types
    ...
    public void Parse_BlankSql_ShouldProvideParameterValidationMessage(int version)
    {
        ...
        var ex = Assert.Throws<ArgumentException>(() => SqlQueryParser.Parse(" ", db, d));
