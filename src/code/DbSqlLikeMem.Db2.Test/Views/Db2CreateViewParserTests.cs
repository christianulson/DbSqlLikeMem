...
/// PT-br: Cobre cenarios de parsing de CREATE VIEW no dialeto Db2.
/// </summary>
...
SELECT * FROM v_users;
";
        var q = SqlQueryParser.ParseMulti(sql, new Db2DbMock(), Get(version, v => new Db2Dialect(v))).ToList();
    ...
    public void Parse_CreateOrReplaceView_ShouldSetFlag(int version)
    {
        ...
        var q = SqlQueryParser.ParseMulti(sql, new Db2DbMock(), Get(version, v => new Db2Dialect(v))).Single();
    ...
    public void Parse_CreateView_WithBackticks_ShouldFail_ByDb2Spec(int version)
    {
        ...
        Assert.ThrowsAny<Exception>(() => SqlQueryParser.ParseMulti(sql, new Db2DbMock(), Get(version, v => new Db2Dialect(v))).ToList());
    }
    ...
    {
        Assert.Throws<InvalidOperationException>(() => SqlQueryParser.Parse("DROP VIEW ;", new Db2DbMock(), Get(version, v => new Db2Dialect(v))));
        Assert.Throws<InvalidOperationException>(() => SqlQueryParser.Parse("DROP VIEW IF EXISTS ;", new Db2DbMock(), Get(version, v => new Db2Dialect(v))));
    }
