...
) : AggregationHavingOrdinalTestsBase<FirebirdDbMock, FirebirdConnectionMock>(helper)
{
    ...
    /// <inheritdoc />
    protected override List<dynamic> Query(string sql)
    {
        var rows = ((DbConnectionMockBase)Connection).QueryRows(sql);
        var dynamicRows = new List<dynamic>(rows.Count);

        foreach (var row in rows)
        {
            var item = new ExpandoObject();
            IDictionary<string, object?> values = item;
            foreach (var pair in row)
                values[pair.Key] = pair.Value;

            dynamicRows.Add(item);
        }
