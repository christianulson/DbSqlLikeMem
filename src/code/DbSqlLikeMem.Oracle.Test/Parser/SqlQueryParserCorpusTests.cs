...
", "WITH inside derived table + outer WHERE" };
    ...
    "CASE expression without ELSE"
        ...
        yield return new object[] {
    "SELECT CAST(id AS NUMBER) FROM users",
...
USING (SELECT id, name FROM users_tmp) src
ON (target.id = src.id)
...
WHEN NOT MATCHED THEN INSERT (id, name) VALUES (src.id, src.name)",
        ...
        yield return new object[] {
            @"MERGE INTO stats target
USING (SELECT grp, SUM(val) AS total FROM t GROUP BY grp) src
...
WHEN NOT MATCHED THEN INSERT (grp, total) VALUES (src.grp, src.total)",
    ...
    "UPDATE multiple columns by id"
        ...
        // MERGE
        yield return new object[] {
            "MERGE INTO users u USING tmp t ON u.id = t.id WHEN MATCHED THEN UPDATE SET u.name = t.name"
        };
    }
