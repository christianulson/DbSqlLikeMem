...
CREATE TABLE {context.TbOrdersFullName} (
    ...
    public override string CrossApplyProjection(FidelityTestContext context) =>
        $"SELECT COUNT(*) FROM {context.TbUsersFullName} u JOIN LATERAL (SELECT o.Note FROM {context.TbOrdersFullName} o WHERE o.{context.TbUsers}Id = u.Id ORDER BY o.Id DESC LIMIT 1) x ON TRUE";

    /// <inheritdoc />
    public override string OuterApplyProjection(FidelityTestContext context) =>
        $"SELECT COUNT(*) FROM {context.TbUsersFullName} u LEFT JOIN LATERAL (SELECT o.Note FROM {context.TbOrdersFullName} o WHERE o.{context.TbUsers}Id = u.Id ORDER BY o.Id DESC LIMIT 1) x ON TRUE";
}
