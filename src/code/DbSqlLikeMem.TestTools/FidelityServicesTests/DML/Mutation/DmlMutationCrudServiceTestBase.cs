...
/// PT-br: Classe base para testes de mutação DML que requerem inserção de parâmetros tipados e verificação dos valores inseridos.
/// </summary>
    ...
    ) : BaseServiceTest(repo, context)
{
    ...
    /// PT-br: Normaliza um valor de string para Oracle quando o provedor é Oracle e o valor é nulo ou vazio. Isso é necessário porque o Oracle trata strings vazias como valores nulos, então este helper garante que quaisquer valores de string vazios sejam convertidos para nulos antes de serem usados em testes que envolvem Oracle. Para outros provedores, o valor é retornado sem alterações.
    /// </summary>
    /// <param name="value">EN: The text value to normalize. PT-br: O valor de texto a normalizar.</param>
    /// <returns>EN: Null for empty Oracle text values; otherwise, the original value. PT-br: Null para valores de texto vazios no Oracle; caso contrario, o valor original.</returns>
    ...
    /// <returns>EN: A completed task once the insert has been verified. PT-br: Uma tarefa concluida quando o insert e validado.</returns>
...
"""
    ...
    /// PT-br: Normaliza um valor DateTime anulavel retornado do banco em uma string legivel ou null.
    /// </summary>
    /// <param name="value">EN: The database value to normalize. PT-br: O valor do banco de dados a normalizar.</param>
    /// <returns>EN: The normalized date-time text, or null when the input is null. PT-br: O texto normalizado da data/hora, ou null quando a entrada e null.</returns>
    ...
    /// PT-br: Tenta normalizar um valor do tipo System.DateOnly para um DateTime com o componente de hora definido para meia-noite. Isso é necessário porque alguns provedores podem retornar valores DateOnly para colunas de data, e este helper permite uma comparação consistente em testes que esperam valores DateTime.
    /// </summary>
    /// <param name="value">EN: The value to inspect for a DateOnly payload. PT-br: O valor a inspecionar para um payload DateOnly.</param>
    /// <param name="dateTime">EN: Receives the normalized DateTime when the value is DateOnly. PT-br: Recebe o DateTime normalizado quando o valor e DateOnly.</param>
    /// <returns>EN: True when the value was normalized from DateOnly; otherwise, false. PT-br: True quando o valor foi normalizado a partir de DateOnly; caso contrario, false.</returns>
    ...
    /// </summary>
    /// <param name="tableName">EN: The table name to inspect. PT-br: O nome da tabela a inspecionar.</param>
    /// <param name="stripped">EN: Receives the base table name when a scenario suffix is found. PT-br: Recebe o nome base da tabela quando um sufixo de cenario e encontrado.</param>
    /// <returns>EN: True when a valid scenario suffix was removed; otherwise, false. PT-br: True quando um sufixo de cenario valido foi removido; caso contrario, false.</returns>
