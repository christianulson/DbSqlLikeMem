...
/// PT-br: Cobre cenarios de commit e rollback de transacao no mock Npgsql.
/// </summary>
    ...
    ) : XUnitTestBase(helper)
{
    ...
    {
        var db = new NpgsqlDbMock();
        using var connA = new NpgsqlConnectionMock(db);
        using var connB = new NpgsqlConnectionMock(db);
        connA.Open();
        connB.Open();
    ...
    public void Close_ShouldPreservePermanentAndGlobalSharedState()
    {
        ...
        using var createGlobalTemp = new NpgsqlCommandMock(connA)
        {
    ...
    /// <summary>
    /// EN: Ensures releasing a savepoint without an active transaction keeps the actionable runtime message in Npgsql.
    /// PT-br: Garante que liberar um savepoint sem uma transação ativa mantenha a mensagem acionável em runtime no Npgsql.
    /// </summary>
    [Fact]
    [Trait("Category", "Strategy")]
    ...
    /// <summary>
    /// EN: Ensures creating a savepoint without an active transaction keeps the actionable runtime message in Npgsql.
    /// PT-br: Garante que criar um savepoint sem uma transacao ativa mantenha a mensagem acionavel em runtime no Npgsql.
    /// </summary>
    [Fact]
    [Trait("Category", "Strategy")]
    ...
    /// <summary>
    /// EN: Ensures rollback to savepoint without an active transaction keeps the actionable runtime message in Npgsql.
    /// PT-br: Garante que rollback para savepoint sem uma transacao ativa mantenha a mensagem acionavel em runtime no Npgsql.
    /// </summary>
    [Fact]
    [Trait("Category", "Strategy")]
    ...
    /// <summary>
    /// EN: Ensures releasing an unknown savepoint keeps the actionable runtime message in Npgsql.
    /// PT-br: Garante que liberar um savepoint desconhecido mantenha a mensagem acionavel em runtime no Npgsql.
    /// </summary>
    [Fact]
    [Trait("Category", "Strategy")]
    ...
    /// <summary>
    /// EN: Ensures creating a savepoint with a blank name keeps the parameter validation message in Npgsql.
    /// PT-br: Garante que criar um savepoint com nome em branco mantenha a mensagem de validacao de parametro no Npgsql.
    /// </summary>
    [Fact]
    [Trait("Category", "Strategy")]
    ...
    /// <summary>
    /// EN: Ensures rolling back to a savepoint with a blank name keeps the parameter validation message in Npgsql.
    /// PT-br: Garante que executar rollback para um savepoint com nome em branco mantenha a mensagem de validacao de parametro no Npgsql.
    /// </summary>
    [Fact]
    [Trait("Category", "Strategy")]
    ...
    /// <summary>
    /// EN: Ensures releasing a savepoint with a blank name keeps the parameter validation message in Npgsql.
    /// PT-br: Garante que liberar um savepoint com nome em branco mantenha a mensagem de validacao de parametro no Npgsql.
    /// </summary>
    [Fact]
    [Trait("Category", "Strategy")]
    ...
    public void ResetAllVolatileData_ShouldClearGlobalTemporaryTableRows()
    {
        ...
        using var createGlobalTemp = new NpgsqlCommandMock(connection)
        {
            CommandText = "CREATE GLOBAL TEMPORARY TABLE gtmp_users AS SELECT id, name FROM users"
        };
        createGlobalTemp.ExecuteNonQuery();
