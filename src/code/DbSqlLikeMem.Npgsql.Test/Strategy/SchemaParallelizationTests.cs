...
/// <summary>
/// EN: Covers schema-level backup and restore when table work runs in parallel on a thread-safe PostgreSQL database.
/// PT-br: Cobre backup e restore em nivel de schema quando o trabalho das tabelas executa em paralelo em um banco PostgreSQL thread-safe.
/// </summary>
    ...
    ) : XUnitTestBase(helper)
{
    ...
    public void BackupRestoreAndClear_ShouldWorkWithMultipleTables_WhenThreadSafe()
    {
        ...
        var schema = Assert.IsType<NpgsqlSchemaMock>(db["DefaultSchema"]);
