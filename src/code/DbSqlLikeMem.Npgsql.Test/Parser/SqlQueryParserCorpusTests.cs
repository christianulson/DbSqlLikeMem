...
/// <summary>
/// EN: Covers parser corpus cases for the Npgsql dialect.
/// PT-br: Cobre casos de corpus do parser para o dialeto Npgsql.
/// </summary>
...
ORDER BY u.Id, o.Amount", "JOIN with derived subquery + ORDER BY multiple keys" };
        ...
        yield return new object[] { "select data->>'name' from users", "JSON_EXTRACT function call" };
...
FROM t
GROUP BY grp
ON CONFLICT (grp) DO UPDATE SET total = EXCLUDED.total",
            "INSERT ... SELECT aggregate with ON CONFLICT DO UPDATE"
        };
...
ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name",
            "INSERT ... SELECT with ON CONFLICT DO UPDATE"
        };
