...
WHERE u.id = 1";
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_ShouldSupportCaseWhenWithMultipleConditionsInAggregations()
    {
        using var connection = CreateFactory().CreateOpenConnection();

        using (var create = connection.CreateCommand())
        {
            create.CommandText = "CREATE TABLE ef_case_multi (id INT PRIMARY KEY, amount INT)";
            _ = create.ExecuteNonQuery();
        }
        ...
        {
            using var insert = connection.CreateCommand();
            insert.CommandText = "INSERT INTO ef_case_multi (id, amount) VALUES (@id, @amount)";
...
FROM ef_case_multi";
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_ShouldSupportParameterizedLikePatternVariants()
    {
        using var connection = CreateFactory().CreateOpenConnection();

        using (var create = connection.CreateCommand())
        {
            create.CommandText = "CREATE TABLE ef_like_variants (id INT PRIMARY KEY, name VARCHAR(100))";
            _ = create.ExecuteNonQuery();
        }
        ...
        {
            using var query = connection.CreateCommand();
            query.CommandText = "SELECT COUNT(*) FROM ef_like_variants WHERE name LIKE @pattern";
            var pattern = query.CreateParameter();
            pattern.ParameterName = "@pattern";
            pattern.Value = value;
            query.Parameters.Add(pattern);
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_ShouldSupportCompositeNullInOrFilterWithPrecedence()
    {
        using var connection = CreateFactory().CreateOpenConnection();

        using (var create = connection.CreateCommand())
        {
            create.CommandText = "CREATE TABLE ef_null_in_or (id INT PRIMARY KEY, nickname VARCHAR(100) NULL, kind VARCHAR(20))";
            _ = create.ExecuteNonQuery();
        }

        using (var i1 = connection.CreateCommand()) { i1.CommandText = "INSERT INTO ef_null_in_or (id, nickname, kind) VALUES (1, NULL, 'staff')"; _ = i1.ExecuteNonQuery(); }
        using (var i2 = connection.CreateCommand()) { i2.CommandText = "INSERT INTO ef_null_in_or (id, nickname, kind) VALUES (2, 'Ana', 'guest')"; _ = i2.ExecuteNonQuery(); }
        using (var i3 = connection.CreateCommand()) { i3.CommandText = "INSERT INTO ef_null_in_or (id, nickname, kind) VALUES (3, 'Bob', 'staff')"; _ = i3.ExecuteNonQuery(); }
        using (var i4 = connection.CreateCommand()) { i4.CommandText = "INSERT INTO ef_null_in_or (id, nickname, kind) VALUES (4, 'Bia', 'guest')"; _ = i4.ExecuteNonQuery(); }

        using var query = connection.CreateCommand();
        query.CommandText = "SELECT COUNT(*) FROM ef_null_in_or WHERE (nickname IS NULL OR nickname IN (@nameA, @nameB)) AND kind = @kind";
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_ScalarSubqueryProjectionWithMultipleInnerRows_ShouldUseFirstCellFromFirstRow()
    {
        using var connection = CreateFactory().CreateOpenConnection();

        using (var createUsers = connection.CreateCommand()) { createUsers.CommandText = "CREATE TABLE ef_scalar_multi_users (id INT PRIMARY KEY, name VARCHAR(100))"; _ = createUsers.ExecuteNonQuery(); }
        using (var createOrders = connection.CreateCommand()) { createOrders.CommandText = "CREATE TABLE ef_scalar_multi_orders (id INT PRIMARY KEY, user_id INT, amount INT)"; _ = createOrders.ExecuteNonQuery(); }
        using (var user = connection.CreateCommand()) { user.CommandText = "INSERT INTO ef_scalar_multi_users (id, name) VALUES (1, 'Alice')"; _ = user.ExecuteNonQuery(); }
        using (var o1 = connection.CreateCommand()) { o1.CommandText = "INSERT INTO ef_scalar_multi_orders (id, user_id, amount) VALUES (10, 1, 25)"; _ = o1.ExecuteNonQuery(); }
        using (var o2 = connection.CreateCommand()) { o2.CommandText = "INSERT INTO ef_scalar_multi_orders (id, user_id, amount) VALUES (11, 1, 30)"; _ = o2.ExecuteNonQuery(); }

        using var query = connection.CreateCommand();
        query.CommandText = @"SELECT
  u.name,
  (SELECT o.amount FROM ef_scalar_multi_orders o WHERE o.user_id = u.id ORDER BY o.id) first_amount
FROM ef_scalar_multi_users u
WHERE u.id = 1";
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_TransactionScope_InsertUpdateDelete_ShouldValidateRollbackThenCommitState()
    {
        using var connection = CreateFactory().CreateOpenConnection();

        using (var create = connection.CreateCommand())
        {
            create.CommandText = "CREATE TABLE ef_tx_sequence (id INT PRIMARY KEY, value INT)";
            _ = create.ExecuteNonQuery();
        }
        ...
        using (var txRollback = connection.BeginTransaction())
        {
            ...
            insert.CommandText = "INSERT INTO ef_tx_sequence (id, value) VALUES (1, 10), (2, 20)";
            _ = insert.ExecuteNonQuery();

            using var update = connection.CreateCommand();
            update.Transaction = txRollback;
            update.CommandText = "UPDATE ef_tx_sequence SET value = value + 5 WHERE id = 1";
            _ = update.ExecuteNonQuery();

            using var delete = connection.CreateCommand();
            delete.Transaction = txRollback;
            delete.CommandText = "DELETE FROM ef_tx_sequence WHERE id = 2";
        ...
        {
            countAfterRollback.CommandText = "SELECT COUNT(*) FROM ef_tx_sequence";
        ...
        using var verify = connection.CreateCommand();
        verify.CommandText = "SELECT value FROM ef_tx_sequence ORDER BY id";
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_PaginationWithDeterministicOrder_ShouldReturnThreeDisjointPagesWithoutGaps()
    {
        using var connection = CreateFactory().CreateOpenConnection();

        using (var create = connection.CreateCommand())
        {
            create.CommandText = "CREATE TABLE ef_page_three (id INT PRIMARY KEY, grp INT)";
            _ = create.ExecuteNonQuery();
        }
        ...
        {
            using var insert = connection.CreateCommand();
            insert.CommandText = "INSERT INTO ef_page_three (id, grp) VALUES (@id, @grp)";
            var id = insert.CreateParameter(); id.ParameterName = "@id"; id.Value = i; insert.Parameters.Add(id);
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_ShouldSupportGroupedCaseWhenWithMultipleBranchesAndElse()
    {
        using var connection = CreateFactory().CreateOpenConnection();

        using (var create = connection.CreateCommand())
        {
            create.CommandText = "CREATE TABLE ef_case_grouped (id INT PRIMARY KEY, category VARCHAR(10), amount INT)";
            _ = create.ExecuteNonQuery();
        }
        ...
        {
            using var insert = connection.CreateCommand();
            insert.CommandText = "INSERT INTO ef_case_grouped (id, category, amount) VALUES (@id, @category, @amount)";
            var id = insert.CreateParameter(); id.ParameterName = "@id"; id.Value = row.Item1; insert.Parameters.Add(id);
...
ORDER BY category";
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_ShouldSupportOrCombinedParameterizedLikePredicates()
    {
        using var connection = CreateFactory().CreateOpenConnection();

        using (var create = connection.CreateCommand())
        {
            create.CommandText = "CREATE TABLE ef_like_or_patterns (id INT PRIMARY KEY, name VARCHAR(100))";
            _ = create.ExecuteNonQuery();
        }
        ...
        {
            using var insert = connection.CreateCommand();
            insert.CommandText = "INSERT INTO ef_like_or_patterns (id, name) VALUES (@id, @name)";
            var id = insert.CreateParameter(); id.ParameterName = "@id"; id.Value = row.Item1; insert.Parameters.Add(id);
            var name = insert.CreateParameter(); name.ParameterName = "@name"; name.Value = row.Item2; insert.Parameters.Add(name);
            _ = insert.ExecuteNonQuery();
        }
        ...
        using var query = connection.CreateCommand();
        query.CommandText = "SELECT COUNT(*) FROM ef_like_or_patterns WHERE name LIKE @containsLi OR name LIKE @threeLetterA";
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_ScalarSubqueryProjectionWithMultipleInnerRows_ShouldBeConsistentPerOuterRow()
    {
        using var connection = CreateFactory().CreateOpenConnection();

        using (var createUsers = connection.CreateCommand()) { createUsers.CommandText = "CREATE TABLE ef_scalar_per_row_users (id INT PRIMARY KEY, name VARCHAR(100))"; _ = createUsers.ExecuteNonQuery(); }
        using (var createOrders = connection.CreateCommand()) { createOrders.CommandText = "CREATE TABLE ef_scalar_per_row_orders (id INT PRIMARY KEY, user_id INT, amount INT)"; _ = createOrders.ExecuteNonQuery(); }

        using (var u1 = connection.CreateCommand()) { u1.CommandText = "INSERT INTO ef_scalar_per_row_users (id, name) VALUES (1, 'Alice')"; _ = u1.ExecuteNonQuery(); }
        using (var u2 = connection.CreateCommand()) { u2.CommandText = "INSERT INTO ef_scalar_per_row_users (id, name) VALUES (2, 'Bob')"; _ = u2.ExecuteNonQuery(); }
        using (var o1 = connection.CreateCommand()) { o1.CommandText = "INSERT INTO ef_scalar_per_row_orders (id, user_id, amount) VALUES (10, 1, 25)"; _ = o1.ExecuteNonQuery(); }
        using (var o2 = connection.CreateCommand()) { o2.CommandText = "INSERT INTO ef_scalar_per_row_orders (id, user_id, amount) VALUES (11, 1, 30)"; _ = o2.ExecuteNonQuery(); }
        using (var o3 = connection.CreateCommand()) { o3.CommandText = "INSERT INTO ef_scalar_per_row_orders (id, user_id, amount) VALUES (20, 2, 40)"; _ = o3.ExecuteNonQuery(); }
        using (var o4 = connection.CreateCommand()) { o4.CommandText = "INSERT INTO ef_scalar_per_row_orders (id, user_id, amount) VALUES (21, 2, 50)"; _ = o4.ExecuteNonQuery(); }

        using var query = connection.CreateCommand();
        query.CommandText = @"SELECT
  u.id,
  (SELECT o.amount FROM ef_scalar_per_row_orders o WHERE o.user_id = u.id ORDER BY o.id) first_amount
FROM ef_scalar_per_row_users u
ORDER BY u.id";
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_CompositeNullInOrFilter_WithoutParentheses_ShouldRespectSqlPrecedence()
    {
        using var connection = CreateFactory().CreateOpenConnection();

        using (var create = connection.CreateCommand())
        {
            create.CommandText = "CREATE TABLE ef_precedence_null_in_or (id INT PRIMARY KEY, nickname VARCHAR(100) NULL, kind VARCHAR(20))";
            _ = create.ExecuteNonQuery();
        }

        using (var i1 = connection.CreateCommand()) { i1.CommandText = "INSERT INTO ef_precedence_null_in_or (id, nickname, kind) VALUES (1, NULL, 'guest')"; _ = i1.ExecuteNonQuery(); }
        using (var i2 = connection.CreateCommand()) { i2.CommandText = "INSERT INTO ef_precedence_null_in_or (id, nickname, kind) VALUES (2, 'Ana', 'staff')"; _ = i2.ExecuteNonQuery(); }
        using (var i3 = connection.CreateCommand()) { i3.CommandText = "INSERT INTO ef_precedence_null_in_or (id, nickname, kind) VALUES (3, 'Bob', 'staff')"; _ = i3.ExecuteNonQuery(); }
        using (var i4 = connection.CreateCommand()) { i4.CommandText = "INSERT INTO ef_precedence_null_in_or (id, nickname, kind) VALUES (4, 'Bia', 'guest')"; _ = i4.ExecuteNonQuery(); }

        using var query = connection.CreateCommand();
        query.CommandText = "SELECT COUNT(*) FROM ef_precedence_null_in_or WHERE nickname IS NULL OR nickname IN (@a, @b) AND kind = @kind";
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_ShouldSupportCaseWhenWithElseInGroupedHavingPredicates()
    {
        using var connection = CreateFactory().CreateOpenConnection();

        using (var create = connection.CreateCommand())
        {
            create.CommandText = "CREATE TABLE ef_case_having_multi (id INT PRIMARY KEY, category VARCHAR(10), amount INT)";
            _ = create.ExecuteNonQuery();
        }
        ...
        {
            using var insert = connection.CreateCommand();
            insert.CommandText = "INSERT INTO ef_case_having_multi (id, category, amount) VALUES (@id, @category, @amount)";
            var id = insert.CreateParameter(); id.ParameterName = "@id"; id.Value = row.Item1; insert.Parameters.Add(id);
            var category = insert.CreateParameter(); category.ParameterName = "@category"; category.Value = row.Item2; insert.Parameters.Add(category);
            var amount = insert.CreateParameter(); amount.ParameterName = "@amount"; amount.Value = row.Item3; insert.Parameters.Add(amount);
            _ = insert.ExecuteNonQuery();
...
ORDER BY category";
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_CompositeNullInOrFilter_SwappedInParameters_ShouldKeepSameResult()
    {
        using var connection = CreateFactory().CreateOpenConnection();

        using (var create = connection.CreateCommand())
        {
            create.CommandText = "CREATE TABLE ef_null_in_or_swapped (id INT PRIMARY KEY, nickname VARCHAR(100) NULL, kind VARCHAR(20))";
            _ = create.ExecuteNonQuery();
        }

        using (var i1 = connection.CreateCommand()) { i1.CommandText = "INSERT INTO ef_null_in_or_swapped (id, nickname, kind) VALUES (1, NULL, 'staff')"; _ = i1.ExecuteNonQuery(); }
        using (var i2 = connection.CreateCommand()) { i2.CommandText = "INSERT INTO ef_null_in_or_swapped (id, nickname, kind) VALUES (2, 'Ana', 'staff')"; _ = i2.ExecuteNonQuery(); }
        using (var i3 = connection.CreateCommand()) { i3.CommandText = "INSERT INTO ef_null_in_or_swapped (id, nickname, kind) VALUES (3, 'Bob', 'staff')"; _ = i3.ExecuteNonQuery(); }
        using (var i4 = connection.CreateCommand()) { i4.CommandText = "INSERT INTO ef_null_in_or_swapped (id, nickname, kind) VALUES (4, 'Bia', 'guest')"; _ = i4.ExecuteNonQuery(); }

        int CountFor(string first, string second)
        {
            using var query = connection.CreateCommand();
            query.CommandText = "SELECT COUNT(*) FROM ef_null_in_or_swapped WHERE (nickname IS NULL OR nickname IN (@a, @b)) AND kind = @kind";
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_PaginationWithDeterministicOrder_ShouldKeepTailContinuityAcrossFourthPage()
    {
        using var connection = CreateFactory().CreateOpenConnection();

        using (var create = connection.CreateCommand())
        {
            create.CommandText = "CREATE TABLE ef_page_tail (id INT PRIMARY KEY, grp INT)";
            _ = create.ExecuteNonQuery();
        }
        ...
        {
            using var insert = connection.CreateCommand();
            insert.CommandText = "INSERT INTO ef_page_tail (id, grp) VALUES (@id, @grp)";
            var id = insert.CreateParameter(); id.ParameterName = "@id"; id.Value = i; insert.Parameters.Add(id);
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_TransactionScope_InsertUpdateDelete_ShouldValidateIntermediateStateBeforeRollbackAndAfterCommit()
    {
        using var connection = CreateFactory().CreateOpenConnection();
        ...
        using (var tx = connection.BeginTransaction())
        {
            ...
            delete.CommandText = "DELETE FROM ef_tx_intermediate WHERE id = 2";
            _ = delete.ExecuteNonQuery();
        ...
        {
            afterRollback.CommandText = "SELECT COUNT(*) FROM ef_tx_intermediate";
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_ShouldSupportGroupedCaseWhenWithCombinedConditionalAggregates()
    {
        using var connection = CreateFactory().CreateOpenConnection();

        using (var create = connection.CreateCommand())
        {
            create.CommandText = "CREATE TABLE ef_case_combined (id INT PRIMARY KEY, category VARCHAR(10), amount INT)";
            _ = create.ExecuteNonQuery();
        }
        ...
        {
            using var insert = connection.CreateCommand();
            insert.CommandText = "INSERT INTO ef_case_combined (id, category, amount) VALUES (@id, @category, @amount)";
            var id = insert.CreateParameter(); id.ParameterName = "@id"; id.Value = row.Item1; insert.Parameters.Add(id);
            var category = insert.CreateParameter(); category.ParameterName = "@category"; category.Value = row.Item2; insert.Parameters.Add(category);
            var amount = insert.CreateParameter(); amount.ParameterName = "@amount"; amount.Value = row.Item3; insert.Parameters.Add(amount);
            _ = insert.ExecuteNonQuery();
...
ORDER BY category";
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_ScalarSubqueryProjection_ShouldMixNonNullAndNullResults()
    {
        using var connection = CreateFactory().CreateOpenConnection();

        using (var createUsers = connection.CreateCommand()) { createUsers.CommandText = "CREATE TABLE ef_scalar_mix_users (id INT PRIMARY KEY, name VARCHAR(100))"; _ = createUsers.ExecuteNonQuery(); }
        using (var createOrders = connection.CreateCommand()) { createOrders.CommandText = "CREATE TABLE ef_scalar_mix_orders (id INT PRIMARY KEY, user_id INT, amount INT)"; _ = createOrders.ExecuteNonQuery(); }
        using (var u1 = connection.CreateCommand()) { u1.CommandText = "INSERT INTO ef_scalar_mix_users (id, name) VALUES (1, 'Alice')"; _ = u1.ExecuteNonQuery(); }
        using (var u2 = connection.CreateCommand()) { u2.CommandText = "INSERT INTO ef_scalar_mix_users (id, name) VALUES (2, 'Bob')"; _ = u2.ExecuteNonQuery(); }
        using (var o1 = connection.CreateCommand()) { o1.CommandText = "INSERT INTO ef_scalar_mix_orders (id, user_id, amount) VALUES (10, 1, 25)"; _ = o1.ExecuteNonQuery(); }

        using var query = connection.CreateCommand();
        query.CommandText = @"SELECT
  u.id,
  (SELECT SUM(o.amount) FROM ef_scalar_mix_orders o WHERE o.user_id = u.id) total_amount
FROM ef_scalar_mix_users u
ORDER BY u.id";
    ...
    [Fact]
    [Trait("Category", "EfCore")]
    public void EfCore_FactoryConnection_PaginationWithDeterministicOrder_ShouldBeRepeatableAcrossThreePages()
    {
        using var connection = CreateFactory().CreateOpenConnection();

        using (var create = connection.CreateCommand())
        {
            create.CommandText = "CREATE TABLE ef_page_repeat_three (id INT PRIMARY KEY, grp INT)";
            _ = create.ExecuteNonQuery();
        }
        ...
        {
            using var insert = connection.CreateCommand();
            insert.CommandText = "INSERT INTO ef_page_repeat_three (id, grp) VALUES (@id, @grp)";
            var id = insert.CreateParameter(); id.ParameterName = "@id"; id.Value = i; insert.Parameters.Add(id);
            var grp = insert.CreateParameter(); grp.ParameterName = "@grp"; grp.Value = i <= 3 ? 1 : i <= 6 ? 2 : 3; insert.Parameters.Add(grp);
            _ = insert.ExecuteNonQuery();
        }
