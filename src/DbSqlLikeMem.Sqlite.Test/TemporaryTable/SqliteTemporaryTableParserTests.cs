...
/// PT: Cobre cenarios de parsing de CREATE TEMPORARY TABLE no dialeto Sqlite.
/// </summary>
...
";
        ...
        var queries = SqlQueryParser.ParseMulti(sql, GetDialect(version, v => new SqliteDialect(v))).ToList();
...
WHERE `tenantid` = 10",
    ...
    public void Parse_ShouldAccept_CreateTemporaryTable_Variants(string sql, int version)
    {
        ...
        var q = SqlQueryParser.Parse(sql, GetDialect(version, v => new SqliteDialect(v)));
        Assert.NotNull(q);
    ...
    public void Parse_ShouldAccept_DropTable_IfExists(int version)
    {
        var q = Assert.IsType<SqlDropTableQuery>(
            SqlQueryParser.Parse("DROP TABLE IF EXISTS tmp_users", GetDialect(version, v => new SqliteDialect(v))));
    ...
    {
        var q = Assert.IsType<SqlDropTableQuery>(
            SqlQueryParser.Parse("DROP GLOBAL TEMPORARY TABLE IF EXISTS tmp_users", GetDialect(version, v => new SqliteDialect(v))));
