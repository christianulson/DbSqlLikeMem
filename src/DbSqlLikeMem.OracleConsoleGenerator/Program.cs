...
#pragma warning restore CA1812
            ...
            : [connections[selectedConnectionIndex - 1]])
        {
            ...
            foreach (var destiny in connInfo.Destinies)
            {
                ...
                var sequences = (destiny.Sequences != null && destiny.Sequences.Count != 0)
                    ? destiny.Sequences
                    : GetSequencesInSchema(connection, destiny.Schema, destiny.Filter);
    ...
    private static List<string> GetSequencesInSchema(IDbConnection cn, string schema, string? filter)
    {
        var qObjects = SqlMetadataQueryFactory.BuildListObjectsQuery(DatabaseType, filter);
        var rows = cn.Query(qObjects, new { databaseName = schema })
            .Select(ToDictionary)
