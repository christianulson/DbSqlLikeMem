...
""",
    ...
    public static string BuildPrimaryKeyQuery(string databaseType)
        => Normalize(databaseType) switch
        {
            "mysql" => "SELECT COLUMN_NAME AS ColumnName FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA=@schemaName AND TABLE_NAME=@objectName AND INDEX_NAME='PRIMARY' ORDER BY SEQ_IN_INDEX;",
            "sqlserver" => """
...
""",
            "sqlite" => "SELECT name AS ColumnName FROM pragma_table_info(@objectName) WHERE pk > 0 ORDER BY pk;",
            "db2" => """
...
""",
    ...
    public static string BuildIndexesQuery(string databaseType)
        => Normalize(databaseType) switch
        {
            "mysql" => "SELECT INDEX_NAME AS IndexName, NON_UNIQUE AS NonUnique, COLUMN_NAME AS ColumnName, SEQ_IN_INDEX AS Seq FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA=@schemaName AND TABLE_NAME=@objectName ORDER BY INDEX_NAME, SEQ_IN_INDEX;",
            "sqlserver" => """
...
""",
            "sqlite" => "-- sqlite indexes fetched through pragma index_list/index_info externally; return empty by default",
            "db2" => """
