...
SELECT * FROM v_users;
";
        var q = SqlQueryParser.ParseMulti(sql, GetDialect(version, v => new Db2Dialect(v))).ToList();
    ...
    public void Parse_CreateOrReplaceView_ShouldSetFlag(int version)
    {
        ...
        var q = SqlQueryParser.ParseMulti(sql, GetDialect(version, v => new Db2Dialect(v))).Single();
    ...
    public void Parse_CreateView_WithBackticks_ShouldFail_ByDb2Spec(int version)
    {
        ...
        Assert.ThrowsAny<Exception>(() => SqlQueryParser.ParseMulti(sql, GetDialect(version, v => new Db2Dialect(v))).ToList());
    }
    ...
    public void Parse_DropView_WithUnexpectedContinuation_ShouldThrow(int version)
    {
        ...
        Assert.Throws<InvalidOperationException>(() => SqlQueryParser.Parse(sql, GetDialect(version, v => new Db2Dialect(v))));
    }
    ...
    {
        Assert.Throws<InvalidOperationException>(() => SqlQueryParser.Parse("DROP VIEW ;", GetDialect(version, v => new Db2Dialect(v))));
        Assert.Throws<InvalidOperationException>(() => SqlQueryParser.Parse("DROP VIEW IF EXISTS ;", GetDialect(version, v => new Db2Dialect(v))));
    }
