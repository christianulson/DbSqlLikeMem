...
WHERE tenantid = 10",
    ...
    public void Parse_ShouldAccept_DropTable_IfExists(int version)
    {
        var q = Assert.IsType<SqlDropTableQuery>(
            SqlQueryParser.Parse("DROP TABLE IF EXISTS tmp_users", new SqlServerDialect(version)));
    ...
    {
        var q = Assert.IsType<SqlDropTableQuery>(
            SqlQueryParser.Parse("DROP GLOBAL TEMPORARY TABLE IF EXISTS tmp_users", new SqlServerDialect(version)));
