...
/// <summary>
/// EN: Covers WHERE expression parsing for the SqlServer dialect.
/// PT: Cobre o parsing de expressoes WHERE para o dialeto SqlServer.
/// </summary>
    ...
    ) : XUnitTestBase(helper)
{
    ...
    public void ParseWhere_ShouldNotThrow_ForSupportedRealWorldExpressions(string whereExpr, int version)
    {
        ...
        var ex = Record.Exception(() => SqlExpressionParser.ParseWhere(whereExpr, new SqlServerDialect(version)));
        Assert.Null(ex);
    }
    ...
    public void Precedence_OR_ShouldBindLooserThan_AND(int version)
    {
        ...
        var ast = SqlExpressionParser.ParseWhere("id = 1 OR id = 2 AND name = 'Bob'", new SqlServerDialect(version));
    ...
    public void Parentheses_ShouldOverridePrecedence(int version)
    {
        ...
        var ast = SqlExpressionParser.ParseWhere("(id = 1 OR id = 2) AND email IS NULL", new SqlServerDialect(version));
