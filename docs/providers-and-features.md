...
- Compatibilidade NHibernate via `UserSuppliedConnectionProvider` com suíte de contrato por provider usando dialeto NHibernate específico por banco. Cobertura atual: SQL nativo com parâmetros, save/get/update/delete de entidade mapeada, rollback transacional, paginação (`FirstResult`/`MaxResults`), consulta HQL e Criteria simples, além de parâmetros nulos e tipos básicos (`string`/`int`/`datetime`/`decimal`) com validação de binding em `INSERT` e `WHERE`.
