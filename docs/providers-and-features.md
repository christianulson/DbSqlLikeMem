...
- Compatibilidade NHibernate via `UserSuppliedConnectionProvider` com suíte de contrato por provider usando dialeto NHibernate específico por banco. Cobertura atual inclui: SQL nativo com parâmetros; save/get/update/delete de entidade mapeada; rollback transacional (SQL nativo e entidade mapeada); paginação (`FirstResult`/`MaxResults`); HQL/Criteria simples; parâmetros nulos e tipados (`string`/`int`/`datetime`/`decimal`) em `INSERT`/`WHERE`; ciclo de vida de sessão (`Evict`, `Clear`, `Merge` em entidade destacada, incluindo garantia de que alterações destacadas após `Evict` não são auto-persistidas no `Flush`, validação de detach completo com `Clear`, recarga de estado persistido com `Refresh` e reconsulta de associação após mutação de relacionamento); relacionamentos many-to-one/one-to-many com atualização de FK por navegação, remoção de filho em coleção e movimentação de filho entre grupos com persistência após `Flush` e nova sessão (incluindo validação SQL da distribuição de FK e garantia de rollback transacional da distribuição original); concorrência otimista (detecção de stale update, incremento de versão após update, `Refresh` para reconciliação de leitura obsoleta e fluxo com duas sessões usando `Refresh`, com validação de incremento de versão a cada commit); além de queries adicionais com projeção parcial em HQL (incluindo aliases via `AliasToEntityMap`, projeção com join+agregação, left join com linha sem relacionamento e contagem escalar por filtro de relacionamento), ordenação por propriedade de relacionamento (incluindo janela paginada determinística) e Criteria com múltiplas restrições (incluindo alias de relacionamento, combinação Conjunction/Disjunction com ordenação determinística e contagem escalar por alias de relacionamento), SaveOrUpdate em entidade detached, FlushMode.Commit com persistência visível no commit, troca múltipla de referência many-to-one na mesma sessão com persistência do FK final, consistência de coleção inversa após reparenting em nova sessão, fluxo stale com refresh+retry controlado (incluindo reaplicação de intenção de negócio após refresh), updates sequenciais alternados entre sessões com validação de incremento previsível de versão, e FlushMode.Manual com/sem Flush explícito, cobrindo persistência e não-persistência no commit, além de associação many-to-one anulável com FK nula persistida, filtro HQL para associação nula, SaveOrUpdate em entidade transient (insert) e comportamento de autoflush em FlushMode.Auto antes de query.
