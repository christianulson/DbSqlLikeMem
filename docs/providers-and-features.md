...
- Inicialização de integração EF Core por provider via fábricas de conexão abertas (`DbSqlLikeMem.<Provider>.EfCore`), cobrindo MySQL, SQL Server, Oracle, PostgreSQL (Npgsql), SQLite e DB2 para uso com providers relacionais do EF Core; com suíte de contrato EF Core segmentada por provider (`DbSqlLikeMem.<Provider>.EfCore.Test`) para validar conexão aberta, fluxo SQL parametrizado, commit/rollback transacional, mutações (`UPDATE`/`DELETE`) e binding de parâmetros nulos/tipados (`decimal`/`datetime`).
- Inicialização de integração LinqToDB por provider via fábricas de conexão abertas (`DbSqlLikeMem.<Provider>.LinqToDb`), cobrindo MySQL, SQL Server, Oracle, PostgreSQL (Npgsql), SQLite e DB2; com suíte de contrato LinqToDB segmentada por provider (`DbSqlLikeMem.<Provider>.LinqToDb.Test`) para validar conexão aberta e fluxo SQL básico parametrizado.
