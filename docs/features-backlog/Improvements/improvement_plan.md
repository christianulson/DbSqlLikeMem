...
**Estado atual:** O snapshot já carrega `CheckConstraints` por tabela e `ComputedExpression` por coluna; a criação de tabelas temporárias, `CREATE TABLE` bruto, `ALTER TABLE ADD COLUMN`, o replay de snapshot e os caminhos de export/import já preservam esses metadados, e o perfil de suporte já reflete `check-constraints` como suportado.

**Plano:**
- Revisar `UnsupportedObjectKinds` quando a cobertura de export/import estiver alinhada com o que o snapshot já consegue round-trip
