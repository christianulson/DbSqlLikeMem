...
- Compatibilidade NHibernate via `UserSuppliedConnectionProvider` com suíte de contrato por provider usando dialeto NHibernate específico por banco.
  ...
  - **Ciclo de vida de sessão/estado**: `Evict`, `Clear`, `Merge` em entidade destacada (incluindo proteção de alterações destacadas após `Evict` e detach completo com `Clear`), `Refresh` com recarga de estado, `Update` vs `Merge` com semântica explícita de reattach/identidade (`Contains`), incluindo conflito de identidade em `Update` (NonUniqueObjectException) e merge sobre instância já gerenciada, `IsDirty`, SaveOrUpdate e FlushMode.
  - **Relacionamentos**: many-to-one/one-to-many com atualização de FK por navegação, remoção de filho em coleção e movimentação de filho entre grupos com persistência após `Flush` e nova sessão; validação SQL de distribuição de FK e preservação da distribuição em rollback; `Cascade.None` sem persistência implícita de filho transient em update de grafo; transições repetidas de associação opcional `null -> group -> null`; exclusão de pai com FK física falha com filhos existentes e passa após dissociação explícita do filho; e mutações apenas da coleção inversa (remoção e adição) não alteram/atribuem FK enquanto many-to-one for o lado dono; por contraste, atribuição no lado dono (many-to-one) persiste FK e reaparece na coleção do pai em nova sessão; em conflitos entre add na coleção inversa e atribuição many-to-one, prevalece o lado dono.
  - **Concorrência otimista**: detecção de stale update, incremento de versão após update, reconciliação com `Refresh` entre sessões, validação de incremento por commit, alternância de updates em três sessões e stale+retry com reaplicação idempotente de regra de negócio.
  - **Queries avançadas**: projeção parcial em HQL (incluindo aliases via `AliasToEntityMap`, join+agregação, left join com linha sem relacionamento e contagem escalar por filtro), ordenação por propriedade de relacionamento com janela paginada determinística, `join fetch` com verificação de inicialização eager (contagem exata de SQL/N+1 documentada como limitação do harness mock), e Criteria com restrições/projeções (incluindo alias de relacionamento, Conjunction/Disjunction, projeção + ordenação + paginação e contagem escalar por alias).
  - **Cenários adicionais de consistência**: associação many-to-one anulável com FK nula persistida, filtro HQL para associação nula, SaveOrUpdate em entidade transient (insert) e autoflush em FlushMode.Auto antes de query.
- Inicialização de integração EF Core por provider via fábricas de conexão abertas (`DbSqlLikeMem.<Provider>.EfCore`), cobrindo MySQL, SQL Server, SQL Azure, Oracle, PostgreSQL (Npgsql), SQLite e DB2 para uso com providers relacionais do EF Core; com suíte de contrato EF Core segmentada por provider (`DbSqlLikeMem.<Provider>.EfCore.Test`) para validar conexão aberta, fluxo SQL parametrizado, commit/rollback transacional (incluindo consistência em múltiplos comandos no mesmo escopo e restauração integral no rollback), mutações (`UPDATE`/`DELETE`), binding de parâmetros nulos/tipados (`decimal`/`datetime`), escalares de agregação (`SUM`/`MAX`), consultas com `IN`/`ORDER BY`, `INNER JOIN`, `EXISTS`, `LEFT JOIN ... IS NULL`, `LIKE` parametrizado com curingas (`%`), filtros compostos com `IS NULL OR ...` (incluindo parâmetro nulo), subquery escalar em `SELECT` (incluindo retorno nulo sem correspondência), cenários `CASE WHEN` em `SELECT` e `HAVING`, leituras transacionais read-after-write no mesmo escopo, além de paginação `OFFSET/FETCH` com ordenação estável por critério determinístico e estabilidade em execuções repetidas da mesma janela e ausência de sobreposição em páginas consecutivas determinísticas.
- Inicialização de integração LinqToDB por provider via fábricas de conexão abertas (`DbSqlLikeMem.<Provider>.LinqToDb`), cobrindo MySQL, SQL Server, SQL Azure, Oracle, PostgreSQL (Npgsql), SQLite e DB2; com suíte de contrato LinqToDB segmentada por provider (`DbSqlLikeMem.<Provider>.LinqToDb.Test`) para validar conexão aberta, fluxo SQL parametrizado, commit/rollback transacional (incluindo consistência em múltiplos comandos no mesmo escopo e restauração integral no rollback), mutações (`UPDATE`/`DELETE`), binding de parâmetros nulos/tipados (`decimal`/`datetime`), escalares de agregação (`SUM`/`MAX`), consultas com `IN`/`ORDER BY`, `INNER JOIN`, `EXISTS`, `LEFT JOIN ... IS NULL`, `LIKE` parametrizado com curingas (`%`), filtros compostos com `IS NULL OR ...` (incluindo parâmetro nulo), subquery escalar em `SELECT` (incluindo retorno nulo sem correspondência), cenários `CASE WHEN` em `SELECT` e `HAVING`, leituras transacionais read-after-write no mesmo escopo, além de paginação `OFFSET/FETCH` com ordenação estável por critério determinístico e estabilidade em execuções repetidas da mesma janela e ausência de sobreposição em páginas consecutivas determinísticas.
- Suíte de contrato compartilhada para Dapper (`DapperSupportTestsBase`) herdada por MySQL, SQL Server, SQL Azure, Oracle, PostgreSQL (Npgsql), SQLite e DB2 para validar agregações com `CASE WHEN` (incluindo `HAVING` parametrizado), composição e variações de `LIKE` parametrizado, filtros compostos (`IS NULL` + `IN` + `OR`, incluindo parâmetro nulo), subquery escalar com múltiplas linhas internas (comportamento mock atual) e sem correspondência (nulo), sequência transacional (`INSERT`/`UPDATE`/`DELETE`) com rollback+commit e leitura transacional read-after-write, além de paginação determinística em páginas consecutivas com repetibilidade entre execuções.
