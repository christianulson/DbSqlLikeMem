...
| 9) `RETURNING`/`OUTPUT`/`RETURNING INTO` | **68%** | ⬆️ | Parser/capabilities evoluíram com suporte explícito a `RETURNING` em `INSERT/UPDATE/DELETE` nos caminhos PostgreSQL e SQLite (incluindo parsing dedicado no gate de dialeto e materialização incremental no executor via `ExecuteReader` para DML básico com projeções diretas/alias/wildcard qualificado como `t.*`, snapshot de linhas afetadas e cobertura de `INSERT ... SELECT ... RETURNING` com múltiplas linhas); payload homogêneo multi-provider, semântica de `OUTPUT` (SQL Server), `RETURNING INTO` (Oracle) e expressões avançadas de `RETURNING` seguem como gap principal. |
...
- **Em evolução:** itens 1, 3, 5, 7, 8, 9 e 10, com impacto recente em JSON/runtime, UPSERT subset e confiabilidade transacional.
