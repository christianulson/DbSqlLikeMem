...
| 7) JOIN/subquery com heurística | **50%** | ⬆️ | Execução multi-tabela e padronização de não suportado avançaram; houve progresso incremental com cache leve por linha externa para subqueries correlacionadas em `EXISTS` e `IN (subquery)` no executor (sem framework novo), mantendo pendente heurística mais ampla de custo/caching. |
