...
- **Em evolução incremental dedicada:** item 6 (plano físico com custo) avançou em heurísticas de cardinalidade, acoplamento `DISTINCT + GROUP BY + ORDER BY` com sensibilidade adicional a `OFFSET`, CASE/JSON em predicados (incluindo funções JSON em nós `FunctionCallExpr`/`CallExpr` e fallback tokenizado para `RawSqlExpr` com sinais de nesting lógico/subquery/JSON), profundidade de nesting lógico com transições `AND/OR`, granularidade de funções agregadas na projeção (incluindo variações de whitespace, agregação `DISTINCT` e `MIN`/`MAX`), custo de subqueries e funções/operadores JSON na projeção por cardinalidade de ocorrência e sensibilidade a múltiplas ocorrências `CASE`/`OVER`, complexidade de expressões em `GROUP BY`/`ORDER BY` (incluindo `UNION`, funções JSON e operadores JSON), CTE com sensibilidade ao formato de fonte interna, sensibilidade a row-limit/offset, fan-out de joins, largura/curinga de projeção e monotonicidade (incluindo `DerivedUnion`), ainda sem mudança arquitetural ampla.
