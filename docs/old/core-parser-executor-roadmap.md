...
| 9) `RETURNING`/`OUTPUT`/`RETURNING INTO` | **58%** | ⬆️ | Parser/capabilities evoluíram com suporte explícito a `RETURNING` em `INSERT/UPDATE/DELETE` no caminho PostgreSQL e gate de rejeição em dialetos sem suporte; no executor PostgreSQL (`ExecuteReader`) há materialização funcional incremental de payload `RETURNING` para DML básico (insert/update/delete) com projeções diretas/alias e snapshot de linhas afetadas, mantendo gap principal no payload homogêneo multi-provider e expressões avançadas de `RETURNING`. |
