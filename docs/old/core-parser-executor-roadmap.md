...
| 4) Tipos/literais/coerção | **66%** | ⬆️ | Base central em `SqlExtensions` e parsing/mapeamento de `DbType` evoluiu com hardening de coerção booleana/textual, `DbType.Time`, preservação de `DateTimeOffset`, suporte ampliado a `Nullable<T>`/enums, cobertura de famílias integrais signed/unsigned, semântica numérica explícita para `VarNumeric` e coerção temporal/booleana consistente em `ToDec`/comparação (`Compare`/`EqualsSql`) para tipos temporais mistos; ainda faltam regras finas por dialeto/versão. |
