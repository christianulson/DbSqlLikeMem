...
| 6) Plano físico com custo | **64%** | ⬆️ | Evolução incremental nas heurísticas do `ExecutionPlan` (custos por formato, cardinalidade de chaves em `GROUP BY`/`ORDER BY`, acoplamento `DISTINCT + GROUP BY + ORDER BY`, IN-list, CASE/JSON em predicados, profundidade de nesting lógico e transições `AND/OR`, granularidade de funções agregadas na projeção com robustez a whitespace e distinção de agregação `DISTINCT` incluindo `MIN`/`MAX`, custo de subqueries na projeção por cardinalidade de ocorrência, sensibilidade a múltiplas ocorrências `CASE`/`OVER` na projeção, custo de funções JSON na projeção, complexidade de expressões em `GROUP BY`/`ORDER BY` e no `ORDER BY` de `UNION` incluindo funções JSON, complexidade de CTE com sensibilidade ao formato de fonte interna, sensibilidade a row-limit/offset, fan-out de joins, largura/curinga de projeção e monotonicidade, incluindo `DerivedUnion`), sem engine física completa. |
...
- **Em evolução incremental dedicada:** item 6 (plano físico com custo) avançou em heurísticas de cardinalidade, acoplamento `DISTINCT + GROUP BY + ORDER BY`, CASE/JSON em predicados, profundidade de nesting lógico com transições `AND/OR`, granularidade de funções agregadas na projeção (incluindo variações de whitespace, agregação `DISTINCT` e `MIN`/`MAX`), custo de subqueries e funções JSON na projeção por cardinalidade de ocorrência e sensibilidade a múltiplas ocorrências `CASE`/`OVER`, complexidade de expressões em `GROUP BY`/`ORDER BY` (incluindo `UNION` e funções JSON), CTE com sensibilidade ao formato de fonte interna, sensibilidade a row-limit/offset, fan-out de joins, largura/curinga de projeção e monotonicidade (incluindo `DerivedUnion`), ainda sem mudança arquitetural ampla.
