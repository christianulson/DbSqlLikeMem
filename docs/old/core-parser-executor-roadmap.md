...
| 4) Tipos/literais/coerção | **58%** | ⬆️ | Base central em `SqlExtensions` e parsing/mapeamento de `DbType` evoluiu com hardening de coerção booleana/textual, `DbType.Time` e preservação de `DateTimeOffset`; ainda faltam regras finas por dialeto/versão. |
