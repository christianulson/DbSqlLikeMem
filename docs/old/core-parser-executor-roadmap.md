...
| 7) JOIN/subquery com heurística | **60%** | ⬆️ | Execução multi-tabela e padronização de não suportado avançaram; houve progresso incremental com cache leve por linha externa para subqueries correlacionadas em `EXISTS`, `IN (subquery)` e subquery escalar no executor, incluindo seleção de chave com foco em identificadores externos realmente referenciados na subquery (evitando variabilidade por colunas não correlacionadas), sem framework novo e mantendo pendente heurística mais ampla de custo/caching. |
