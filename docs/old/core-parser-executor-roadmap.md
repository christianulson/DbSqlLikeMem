...
| 4) Tipos/literais/coerção | **60%** | ⬆️ | Base central em `SqlExtensions` e parsing/mapeamento de `DbType` evoluiu com hardening de coerção booleana/textual, `DbType.Time`, preservação de `DateTimeOffset`, suporte ampliado a `Nullable<T>`/enums e parsing de tipos numéricos/string adicionais; ainda faltam regras finas por dialeto/versão. |
