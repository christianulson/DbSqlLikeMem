...
| 4) Tipos/literais/coerção | **77%** | ⬆️ | Base central em `SqlExtensions` e parsing/mapeamento de `DbType` evoluiu com hardening de coerção booleana/textual, `DbType.Time`, preservação de `DateTimeOffset`, suporte ampliado a `Nullable<T>`/enums, cobertura de famílias integrais signed/unsigned, semântica numérica explícita para `VarNumeric`, coerção temporal/booleana consistente em `ToDec`/comparação (`Compare`/`EqualsSql`) para tipos temporais mistos, semântica binária por conteúdo/ordenação lexicográfica para `byte[]`, parsing mais abrangente de `DbType.Object` (incluindo `Guid`/`DateTimeOffset`/`DateTime`/`TimeSpan`) e binário hexadecimal (`0x...`, `\\x...`, `X'...'`), além de tratamento consistente de `NULL` textual quoted por tipo, aliases booleanos compactos (`t/f`) e coerção explícita para famílias integrais `sbyte/ushort/uint/ulong` em `ToBool`/`ToDec`; ainda faltam regras finas por dialeto/versão. |
...
| 9) `RETURNING`/`OUTPUT`/`RETURNING INTO` | **50%** | ⬆️ | Parser/capabilities evoluíram com suporte explícito a `RETURNING` em `INSERT/UPDATE/DELETE` no caminho PostgreSQL e gate de rejeição em dialetos sem suporte; payload homogêneo multi-provider no executor continua como principal gap. |
