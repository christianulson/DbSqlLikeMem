...
| 4) Tipos/literais/coerção | **62%** | ⬆️ | Base central em `SqlExtensions` e parsing/mapeamento de `DbType` evoluiu com hardening de coerção booleana/textual, `DbType.Time`, preservação de `DateTimeOffset`, suporte ampliado a `Nullable<T>`/enums, cobertura de famílias integrais signed/unsigned e semântica numérica explícita para `VarNumeric`; ainda faltam regras finas por dialeto/versão. |
