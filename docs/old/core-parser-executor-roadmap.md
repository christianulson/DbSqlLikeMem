...
- **Em evolução incremental dedicada:** item 6 (plano físico com custo) avançou em heurísticas de cardinalidade, acoplamento `DISTINCT + GROUP BY + ORDER BY`, CASE/JSON em predicados, profundidade de nesting lógico com transições `AND/OR`, granularidade de funções agregadas na projeção (incluindo variações de whitespace), CTE com sensibilidade ao formato de fonte interna, sensibilidade a row-limit/offset, fan-out de joins, largura/curinga de projeção e monotonicidade (incluindo `DerivedUnion`), ainda sem mudança arquitetural ampla.
