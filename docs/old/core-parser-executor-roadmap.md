...
| 3) UPSERT por família de banco | **74%** | ⬆️ | `ON DUPLICATE`/`ON CONFLICT` e subset de `MERGE` avançaram; executor agora diferencia `ON CONFLICT ... DO NOTHING` e o subset de `MERGE` ficou mais robusto para alias de source sem `AS`, restando harmonizações finais de semântica por provider. |
